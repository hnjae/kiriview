...
const ProviderGenerationState& providerFor(const std::array<ViewportEngineRoleState, 2>& roles,
    ImageViewport::PageRole role) { return roles[role == ImageViewport::PageRole::Secondary ? 1U : 0U].provider; }
void publishSecondary(RequestState& request, DisplayState& display, const ProviderGenerationState& provider)
{
    if (!hasSecondary(request)) { display.roles[1] = {}; return; }
    ...
    shown.displayedImageSize = request.roles[1].provider ? provider.logicalSize : sourceLogicalSize(request.roles[1].source);
    if (!request.roles[1].provider && !shown.pendingRenderPayload.image.isNull()) {
        shown.displayedImage = shown.pendingRenderPayload.image; shown.displayedPayload = shown.pendingRenderPayload;
    }
}
void publishReady(RequestState& request, DisplayState& display,
    const ProviderGenerationState& primaryProvider, const ProviderGenerationState& secondaryProvider,
    const PreparedPayload& payload)
{
    request.status = ImageViewport::RequestStatus::Ready; request.reason = ImageViewport::RequestReason::Ready;
    display.status = ImageViewport::DisplayStatus::Ready;
    if (request.roles[0].source.facts.provider) display.commitPreparedPayloadIdentity(request.roles[0].activeRequest, payload);
    ...
    const int position = request.roles[0].activeRequest.resolvedFrame.position >= 0
        ...
        : request.roles[0].source.facts.provider ? primaryProvider.timingIntervals.frameStartPosition(frame)
                                                : sourceFrameStartPosition(request.roles[0].source, frame);
    display.roles[0].displayedRequest = display.activeRequestSnapshot(request.sequenceGeneration, request.roles[0].activeRequest, position);
    display.roles[0].displayedImageSize = request.roles[0].source.facts.provider ? primaryProvider.logicalSize : sourceLogicalSize(request.roles[0].source);
    display.roles[0].displayedImage = payload.image; display.roles[0].displayedPayload = payload;
    publishSecondary(request, display, secondaryProvider);
}
void markPlayback(ViewportChangeSet& changes, PlaybackState& playback, ImageViewport::PlaybackPhase phase)
{
    if (playback.phase == phase) return;
    playback.phase = phase; changes.playbackPhase = true; changes.requestState = true; changes.requestRevision = true;
}
}

ViewportRenderSynchronization synchronizeViewportEngineRender(
    ViewportEngineRenderSynchronizationInput input, ViewportEngineRenderSynchronizationAccess access)
{
    ...
    result.pendingTargetCommit = !terminalSealed(access.request()) && waitingForRender(access.request())
    ...
    if (result.pendingTargetCommit) result.preparedPayload = access.display().roles[0].pendingRenderPayload;
    ...
    result.geometryState = projectViewportGeometryState(
        result.pendingTargetCommit ? input.pendingGeometry : input.currentGeometry, access.presentation());
    result.renderSnapshot = projectViewportRenderSnapshot(
        { input.itemSize, result.pendingTargetCommit, result.preparedPayload, result.geometryState },
    ...
    ViewportEngineRenderAcknowledgementInput input, ViewportEngineRenderCommitAccess access)
{
    ...
    if (terminalSealed(access.request()) || !input.renderedImagePresent
        ...
        || !ViewportEngineRenderAcknowledgement::completeMatches(access.display(), access.request(), input.acknowledgement)) return result;
    const auto oldStatus = access.display().status;
    if (input.pendingTargetCommit) publishReady(access.request(), access.display(),
        providerFor(access.roles(), ImageViewport::PageRole::Primary),
        providerFor(access.roles(), ImageViewport::PageRole::Secondary), input.preparedPayload);
    if (input.pendingSecondaryProviderCommit) {
        access.display().roles[1].displayedImage = access.display().roles[1].pendingRenderPayload.image;
        access.display().roles[1].displayedPayload = access.display().roles[1].pendingRenderPayload;
        access.display().roles[1].displayedImageSize = providerFor(access.roles(), ImageViewport::PageRole::Secondary).logicalSize;
    }
    ...
    access.display().commitDisplayedRequestSnapshot(access.request().sequenceGeneration,
        access.request().roles[0].activeRequest, access.display().roles[0].pendingRenderPayload.payloadId);
    access.display().clearPendingRenderPayload(); access.display().clearRenderFailureRetainedDisplay();
    if (resume) {
        markPlayback(changes, access.playback(), access.playback().stopWhenRequestReady
            ? ImageViewport::PlaybackPhase::Stopped : ImageViewport::PlaybackPhase::Playing);
        access.playback().stopWhenRequestReady = false;
    }
    if (input.pendingTargetCommit) {
        changes.requestState = true; changes.requestRevision = true; changes.displayRevision = true;
        changes.displayState = access.display().status != oldStatus;
        changes.geometryState = rectsDifferExactly(PresentationGeometry::contentRect(input.geometryState), input.oldContentRect)
            || rectsDifferExactly(PresentationGeometry::visibleImageRect(input.geometryState), input.oldVisibleImageRect);
    }
    return result;
}
    ...
    ViewportEngineRenderAcknowledgementInput input, ViewportEngineRenderFailureAccess access)
{
    ...
    if (terminalSealed(access.request())
        || (input.acknowledgement.synchronizationAttempt != 0
            && input.acknowledgement.synchronizationAttempt != access.render().nextSynchronizationAttempt)
        || !ViewportEngineRenderAcknowledgement::failureMatches(
            access.display(), access.request(), input.acknowledgement)
        || (access.display().status != ImageViewport::DisplayStatus::Ready && !pending)) return result;
    const auto oldStatus = access.display().status;
    ...
    if (access.display().roles[0].retainedDisplayValid) {
        ...
        for (auto& role : access.display().roles) {
            if (role.retainedDisplayValid) {
                role.displayedRequest = role.retainedRequest; role.displayedImageSize = role.retainedImageSize;
    ...
    terminal.clear(); terminal.sealed = true; terminal.generation = access.request().sequenceGeneration;
    ...
    role.terminal = true; role.status = ImageViewport::RequestStatus::Error;
    role.reason = ImageViewport::RequestReason::RenderFailure; role.failureScope = FailureScope::DisplayRequest;
    role.diagnostic = QStringLiteral("render commit failed");
    access.request().status = role.status; access.request().reason = role.reason;
    ...
    changes.requestState = true; changes.requestRevision = true; changes.diagnostics = diagnosticChanged;
    changes.displayRevision = true; changes.displayState = access.display().status != oldStatus;
