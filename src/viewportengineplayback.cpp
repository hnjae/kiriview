    ...
    const PlaybackCommandInput& input)
{
    ...
    auto beginRoleRequest =
        [this](ImageViewport::PageRole role, ImageViewportInternal::DisplayRequestOrigin origin,
            ImageViewportInternal::DisplayRequestTarget target,
            ImageViewportInternal::ResolvedFrameIdentity resolved, bool remember) {
    ...
    auto dispatchProvider = [this, &result, &markRequest, &input](ImageViewport::PageRole role,
        ...
        if (provider.requests.activeFrameToken.isValid()) {
            const auto queued
                = queueProviderFrameRequest({ role, target.frame, target.providerTargetKind });
    ...
    if (input.command.kind == ViewportPlaybackCommand::Kind::Pause
        ...
        const bool preservePosition = playbackAccess().playback().role == input.command.role
        ...
        playbackAccess().playback().stopWhenRequestReady = false;
        if (!preservePosition) {
            const int frame = requestRole(playbackAccess().request(), input.command.role)
                                  .activeRequest.target.frame;
            const auto& intervals = source.facts.provider ? provider.facts.timingIntervals
        ...
        if (source.facts.provider
            && (playbackAccess().request().status == ImageViewport::RequestStatus::Unsupported
                || playbackAccess().request().status == ImageViewport::RequestStatus::Error)) {
            auto& active
                = requestRole(playbackAccess().request(), input.command.role).activeRequest;
            ...
            active.resolvedFrame
                = { frame, provider.facts.timingIntervals.frameStartPosition(frame) };
