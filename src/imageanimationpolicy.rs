...
mod ffi {
    ...
    extern "Rust" {
        ...
        #[cxx_name = "rustAnimationFrameDelayFromTimescale"]
        fn rust_animation_frame_delay_from_timescale(duration: u32, timescale: u32) -> i32;

        #[cxx_name = "rustAnimationHasRemainingLoops"]
        fn rust_animation_has_remaining_loops(state: RustAnimationLoopState) -> bool;

        #[cxx_name = "rustAdvanceAnimationLoop"]
        fn rust_advance_animation_loop(state: RustAnimationLoopState) -> RustAnimationLoopAdvance;

        #[cxx_name = "rustAdvanceDecodedAnimation"]
        fn rust_advance_decoded_animation(
            frame_count: usize,
            frame_index: usize,
...
mod tests {
    ...
    #[test]
    fn normalized_frame_delay_uses_default_for_negative_delays() {
        assert_eq!(rust_normalized_animation_frame_delay(-1), 100);
    }

    #[test]
    fn normalized_frame_delay_applies_minimum_to_short_delays() {
    ...
    #[test]
    fn timescale_frame_delay_rounds_up_duration_to_milliseconds() {
        assert_eq!(rust_animation_frame_delay_from_timescale(1, 24), 42);
        assert_eq!(rust_animation_frame_delay_from_timescale(3, 2), 1500);
    }

    #[test]
    fn timescale_frame_delay_rejects_missing_duration_or_timescale() {
        assert_eq!(rust_animation_frame_delay_from_timescale(0, 24), 0);
        assert_eq!(rust_animation_frame_delay_from_timescale(1, 0), 0);
    }

    #[test]
    fn timescale_frame_delay_clamps_to_timer_range() {
        assert_eq!(
            rust_animation_frame_delay_from_timescale(u32::MAX, 1),
            i32::MAX
        );
    }
    ...
    fn loop_advance_increments_only_when_more_loops_are_available() {
        let advance = rust_advance_animation_loop(loop_state(2, 1));
        assert!(advance.should_continue);
        assert_eq!(advance.completed_loops, 2);

        let stop = rust_advance_animation_loop(loop_state(2, 2));
        assert!(!stop.should_continue);
        assert_eq!(stop.completed_loops, 2);
    }

    #[test]
    fn loop_advance_saturates_completed_loop_count() {
        let advance = rust_advance_animation_loop(loop_state(-1, i32::MAX));
        assert!(advance.should_continue);
        assert_eq!(advance.completed_loops, i32::MAX);
    }

    #[test]
    fn decoded_animation_advance_steps_to_next_frame() {
        let advance = rust_advance_decoded_animation(3, 0, loop_state(0, 0));
    ...
    fn decoded_animation_advance_loops_after_last_frame_when_available() {
        let advance = rust_advance_decoded_animation(2, 1, loop_state(1, 0));
    ...
    fn decoded_animation_advance_stops_after_last_frame_without_remaining_loops() {
        let advance = rust_advance_decoded_animation(2, 1, loop_state(0, 0));
    ...
    fn decoded_animation_advance_rejects_missing_or_invalid_frame_state() {
        assert!(!rust_advance_decoded_animation(0, 0, loop_state(0, 0)).frame_available);
        assert!(!rust_advance_decoded_animation(2, 2, loop_state(0, 0)).frame_available);
    }
}
