...
struct TileKey
{
    ...
    friend bool operator!=(TileKey left, TileKey right) { return !(left == right); }
};

uint qHash(TileKey key, uint seed = 0);
...
public:
    ...
    bool containsTile(TileKey key) const;
    int selectLevelForDisplayScale(qreal displayPixelsPerSourcePixel) const;
    QRect levelTileRect(TileKey key) const;
    QRect levelTileTextureRect(TileKey key) const;
    QRect sourceRectForLevelRect(int level, QRect levelRect) const;
    TileRequest requestForTile(TileKey key) const;
    std::vector<TileKey> tilesIntersectingLevelRect(int level, QRect levelRect) const;
