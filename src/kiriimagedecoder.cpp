...
std::optional<KiriView::DecodedImageResult> decodeHeifSequenceImageData(const QByteArray &data)
{
    ...
    if (d->track == nullptr) {
        if (errorString != nullptr) {
            *errorString = imageViewText(
                "Could not decode the selected HEIF image: sequence track is missing.");
        }
        return std::nullopt;
    }
