    ...
    ViewportEngineProviderDemandInput input, ViewportEngineProviderDemandProjectionAccess access)
{
    ...
    const auto& provider = access.roles()[index].provider;
    const auto& payload = access.display().roles[index].displayedPayload;
    QSizeF logicalSize = provider.logicalSize;
    if (logicalSize.isEmpty()) logicalSize = sourceLogicalSize(source);
    ...
    if (!logicalBounds.isEmpty()) visibleRect = visibleRect.intersected(logicalBounds);
    QSizeF targetPixels = PresentationGeometry::pageItemRect(projected, input.role).size();
    if (targetPixels.isValid() && projected.devicePixelRatio > 0.0) targetPixels *= projected.devicePixelRatio;
    ...
    demand.setRequestedPosition(request.target.providerTargetKind == ProviderRequestTargetKind::Frame
        ? request.resolvedFrame.position : request.target.position);
