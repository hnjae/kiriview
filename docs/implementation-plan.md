...
- Completion evidence: Added `DisplayRequest::resolvedFrame` so public target and provider/preparation frame identity are separate, routed provider command frame selection and frame admission through that resolved identity, carried resolved identity through metadata-bound selection, provider queue validation, render/display snapshots, and stop restoration ready-display matching, and updated the provider frame admission test to assert resolved-frame identity. Removed duplicated provider active-frame request id/playback/target-kind state so active provider frame scope is derived from `DisplayRequest::providerFrameToken` and the active request target origin/kind. Added active request/display ownership helpers for provider frame tokens and prepared payload identities, routed render commit/failure acknowledgement matching through those helpers, and added `staleRenderCommitAcknowledgementIsIgnoredWithoutSceneGraph()` to prove stale commit acknowledgements do not change public state, revisions, or pending render ownership. Verified focused `imagesequence_factory`, `imageviewport_provider_requests`, `imageviewport_provider_playback`, `imageviewport_provider_terminal`, `imageviewport_provider_lifecycle`, and `imageviewport_render_commit`, plus full `devenv shell -- just test` and `devenv shell -- just lint`.
