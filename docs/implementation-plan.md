...
- Status: In progress.
- Progress: secondary `pause(role)` and `stop(role)` now admit present non-driver roles as accepted no-ops without mutating the active primary playback driver; secondary `play(role)`, `seek(role, frame)`, `seekToPosition(role, milliseconds)`, provider role-driver advancement, and authored animation policy remain.
