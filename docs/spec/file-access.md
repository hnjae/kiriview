...
KiriView may internally resolve a KIO-backed direct video URL to a playback URL before handing it to the video backend. KiriView treats this resolution as successful only when the resolved playback URL can be consumed by the video backend. This does not change the user-facing source URL for the window title, adjacent direct media navigation, deletion target, error context, or direct-media versus opened-collection routing decisions.
