...
The active navigation thumbnail strip may display generated preview thumbnails for supported direct local image items and supported image pages inside directly opened CBZ or CB7 archive collections whose archive entries provide recorded content checksum metadata. Unsupported, pending, failed, direct video, archive-entry media, non-CBZ or non-CB7 archive-collection, CBZ or CB7 entries without usable checksum metadata, and directory-collection items keep placeholder media-type icons instead of generated preview thumbnails.
...
After visible, nearby, and user-navigation thumbnail demand for the current active navigation list has been satisfied, KiriView may silently fill supported direct local image thumbnails and supported archive-metadata-backed image page thumbnails from directly opened CBZ or CB7 archive collections for that active list in the background. Visible, nearby, and user-navigation demand always takes precedence over this background fill work, and new foreground demand, navigation changes, or thumbnail-list resets may cancel background work already in progress. Background fill failures do not show user-facing errors and keep the normal thumbnail fallback UI.
