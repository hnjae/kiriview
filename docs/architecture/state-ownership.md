...
- Ordinary direct media predecode: the C++ document session owns the predecode/cache lifetime for mixed image/video media scopes. Video items may be used as cursor positions for image-predecode priority, but cached runtime objects remain still-image payloads and video frames are not predecoded.
