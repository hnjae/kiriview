...
An open demand window stages facts without admitting work; closing the matching-generation window atomically replaces committed demand. Reports outside a valid open window are rejected. Current-row and visible work are admitted together and may run in parallel. Nearby work may run in parallel only after all current-row and visible work is neither pending nor active. Background fill remains single-work and may run only after all demand work is neither pending nor active. Newly committed higher-tier demand cancels active lower-tier or background work before it is admitted.

A higher-priority demand for the same source key and bucket promotes retention and its existing claim without changing the work id or thumbnail navigation generation. A lower-priority or expired demand must not demote already visible/current demand until a newer demand-window epoch says that row is no longer visible/current. A current row with an already committed or staged bucket is foreground even when its QML geometry report is nearby; a current row without a bucket waits for the next demand-window report rather than inventing a size.
