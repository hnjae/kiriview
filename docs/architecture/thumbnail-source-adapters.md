...
`ActiveNavigationThumbnailRowStore` owns durable row identity, thumbnail navigation generation, current-row projection, the source-revision mutation index, model publication, result projection, and the acquisition, replacement, reprioritization, and release of `ThumbnailImageStore` entries. Row-list input crosses a two-phase transaction: preparation classifies the proposed rows without mutation, then commit applies exactly that prepared value. The row store compares normalized durable row keys rather than raw requested-URL spelling and classifies a matching durable row list as projection-only or source-payload refresh; row order or durable-key changes are identity replacement.

`ActiveNavigationThumbnailScheduler` owns the immutable scheduling row snapshot and typed demand and source-revision indexes, source adapter, committed demand snapshot epoch, current-row input, accepted demand, incomplete-demand tier indexes, active-work tier indexes, completed background buckets, stable work ids, row claims, the background candidate cursor and sweep bound, and foreground, nearby, and background admission. It returns an ordered closed variant of cancel, start, projection, retention-class, accepted-completion, and queued-continuation effects; each alternative carries only its valid payload. It has no access to QObject state, the row store, image store, model, executor class, or providers. A work id is allocated once for an admitted cache lookup or generation request and remains unchanged when a cache miss advances that work into generation.

`ActiveNavigationThumbnailWorkCoordinator` visits scheduler effect alternatives in order, delegates starts and cancellation to the job executor, publishes accepted completions through the row port, installs ready images, maps scheduler-owned visible, nearby, and background retention classes into image-store priorities, records failure diagnostics, and delivers scheduler-requested admission continuations through the Qt event queue. The scheduler supplies an admission epoch with each continuation, so invalidation or reset makes an already queued callback inert. The coordinator does not retain independent demand, work claim, cursor, admission epoch, or effect-kind state.

Neutral thumbnail-work values own the plain request, result, and completion shapes shared by scheduling and execution. `ActiveNavigationThumbnailJobExecutor` owns only cache-lookup and generation providers, backend phase tokens, cancelable backend jobs, the cache-miss-to-generation chain, phase-level stale-completion rejection, callback dispatch, and callback lifetime safety. It accepts work requests and returns typed completions carrying the original work id, source key, bucket, and work kind. It does not define scheduling effects or access row storage, model publication, the image store, retention policy, demand snapshots, or failure diagnostics.
...
A prepared row commit returns its current row and, when scheduling rows changed, one sealed scheduling snapshot containing the navigation generation and all source-revision rows. Scheduler reset accepts only a snapshot whose valid source-revision and demand identities are unique and whose row generations all equal the snapshot generation. A source-payload refresh preserves generation and accepted work while replacing equivalent requested-URL payloads used by future adapter work. Separate source-key and generation arguments are not scheduler or coordinator APIs.

A current-row-only change preserves thumbnail navigation generation, accepted results, and provider entries. When row identity changes, the runtime prepares the complete update, invalidates coordinator work and demand identity, then commits the prepared rows so the row store releases provider entries, advances thumbnail navigation generation, rebuilds the source-revision index, and publishes the new model snapshot. Preparation itself must not release entries, advance generation, or publish model state. Projection-only and source-payload refresh commits do not invalidate work.

The document-session projection owner retains a thumbnail dependency identity for valid and unavailable row sets. Repeating the same unavailable identity is inert, a current-row-only dependency change updates only current projection, and candidate source or revision changes rebuild rows even when the public session projection is unchanged. Public snapshot changes outside active-navigation and candidate dependencies do not invoke thumbnail row projection.

Every executor completion is accepted by the scheduler only after matching the active work id, generation-scoped source revision key, demand bucket, and work kind. The executor separately accepts provider callbacks only when their private backend phase token still matches the active work phase. Only a completion accepted at both boundaries may be published by the coordinator through the row port. A background completion does not replace an existing usable foreground-ready image, and a failed larger foreground request preserves an existing usable ready image.
...
Background fill may run only when no visible, current-row, or nearby work is pending or active. It uses a bounded cursor over eligible placeholder rows and bucket levels; when one bounded step finds no eligible candidate but the sweep remains incomplete, the scheduler requests one queued continuation and resumes from that cursor on the next event-loop turn. A new valid demand snapshot rearms a complete background sweep, while reset or invalidation advances the admission epoch and rejects continuations from the prior row identity. Foreground demand is admitted immediately without waiting for a queued background continuation. Demand replacement visits the reported and previously accepted demand rows, current changes reclassify only the previous and next current rows, and admission visits indexed incomplete or active tiers instead of synchronously traversing the full active-navigation row list after each demand report, completion, or current-row update.
