...
`ActiveNavigationThumbnailWorkCoordinator` owns the source adapter, demand deduplication and demand-window epoch, accepted foreground demand and background bucket state, foreground and background coordination, stable work ids, row claims, completion publication, and failure diagnostics. A work id is allocated once for an accepted cache lookup or generation request and remains unchanged when a cache miss advances that work into generation.
...
Every executor completion is accepted by the coordinator only after matching the active work id, source key including thumbnail navigation generation, demand bucket, and work kind. The executor separately accepts provider callbacks only when their private backend phase token still matches the active work phase. Only a completion accepted at both boundaries may cross the row port. A background completion does not replace an existing usable foreground-ready image, and a failed larger foreground request preserves an existing usable ready image.
