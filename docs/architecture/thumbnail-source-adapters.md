...
`ActiveNavigationThumbnailRowStore` owns durable row identity, thumbnail navigation generation, current-row projection, typed demand and source-revision indexes, model publication, result projection, and the acquisition, replacement, reprioritization, and release of `ThumbnailImageStore` entries.
