...
Architecture boundary tests should fail when code reintroduces a second owner for durable public state. Prefer simple source-pattern checks for QML and facade API boundaries, and focused Qt tests for runtime behavior that depends on signal ordering, object lifetime, or physical Qt Quick item adaptation.
