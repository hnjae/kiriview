...
Viewport scan-start state, pan/scan command planning, anchored zoom positioning, and viewport point-query geometry should belong to the image-document runtime/presentation layer, not the QML facade. The facade may keep forwarding QML commands while the runtime owns pending displayed-image start state, updates it from document change publication, and computes viewport content-position commands from presentation state.
