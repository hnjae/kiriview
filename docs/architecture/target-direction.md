...
Image document synchronization should be a named document-session planner. The planner should decide, from committed session facts, image leaf snapshots, and the direct-image cursor sync result, whether an image leaf change updates session source identity, mirrors image deletion progress, refreshes or caches direct-media navigation, publishes image-page active navigation, or recomputes the public projection. `DocumentSessionRuntime` applies the returned operations, but it must not inline those leaf-change follow-up decisions.

Video document synchronization should be a named document-session planner. The planner should decide whether a video leaf snapshot leaves the session unchanged, clears session direct-media state for an empty source, or commits a direct-video cursor from the leaf source. `DocumentSessionRuntime` applies the returned operations to `DocumentSessionState`, but it must not inline empty-source/direct-video branching.

Direct-media navigation application should be a named document-session subowner. The application runtime should translate inactive-refresh clearing and refresh/open workflow applications into state replacement, reveal-context operations, projection publication, predecode clearing or scheduling, and follow-up media routes through narrow ports. `DocumentSessionRuntime` may supply current cursor and active-navigation facts, but it must not inline workflow-application branching or own the effect sequence.
