...
Explicit seek acceptance is source-neutral: the controller records the accepted target, latest non-playback target, diagnostics reset, and playback waiting projection once, then backend materialization either dispatches a provider request, waits for provider metadata, or publishes an in-memory frame.
