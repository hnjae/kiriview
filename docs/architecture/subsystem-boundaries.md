...
The canonical store is decomposed by invariant rather than by operation file. Accepted target and source identity, request lifecycle and terminal projection, playback lifecycle, visible and pending display, per-role provider sessions, presentation preferences, render coordination, command diagnostics, and revision allocation are distinct state components under the single engine owner. Identities and counters live with the component that allocates them; a convenience aggregate must not merge these components into a broadly mutable request or controller state.

Capability selection is transition-specific, not merely domain-specific. A command or event discriminator is validated before the facade constructs the capability for its selected operation. Operations that only observe a component receive a const view, and cross-domain transactions receive an explicit composite capability listing each mutable component. A playback command does not acquire every playback-related component merely because another playback command needs it, and provider event variants do not share one maximum-authority provider capability.
...
Every state-changing facade result contains its coherent change set, provider transport ordered before and after publication, playback scheduling intent, and internal diagnostics. Revision finalization remains a two-phase engine operation because before-publication provider transport may synchronously re-enter the engine. The engine returns a move-only pending-publication value that only the originating engine can consume; consuming nested values in actual publication order allocates revisions without exposing raw revision or change-set mutation ports.
