...
- Split-brain state: viewport scroll position and logical viewport frame are owned on both sides of the QML/C++ boundary in `src/qml/ImageViewport.qml` and `src/presentation/imagespreadpresentationcontroller.cpp`; QML owns `Flickable.contentX/Y`, C++ owns `viewportContentPosition` and `visibleItemRect`, and `Qt.callLater` plus local generations reconcile them after the fact.
- Double ownership: spread zoom is owned by the spread zoom controller while primary and secondary page zoom state remain separately mutable in `src/presentation/imagespreadzoomcontroller.cpp`, `src/presentation/imagepresentationviewportstate.cpp`, and `src/presentation/imagepresentationcontroller.cpp`; mode switches copy zoom between owners instead of activating one canonical owner.
