...
The C++ document runtime owns a `MediaEntrySourceStore` for default archive access. The store tracks the current `OpenedCollectionScopeLocation`, caches the sorted candidate list, and routes archive image reads through one serialized media-entry-source runner.

Media entry sources are owned below `archivebackend`:
...
The source is discarded when the document runtime leaves that archive collection: opening a different archive collection, opening direct media, clearing the image document, successfully deleting the current collection, or shutting down.

## Consequences

Archive collection navigation and predecode reuse the candidate list and archive storage already opened for the current source. This keeps the archive backend open count to one filesystem-level open per active archive collection source in the default runtime path.
