...
- Evidence: `src/async/imageiojobs.h:7-13` imports directory listing, callbacks, worker scheduler, image location, and navigation candidate types. `src/async/imageiojobs.cpp:58-128` still implements directory candidates and opened-collection candidate loading. `src/navigation/imagedocumentpagecandidateprovider.cpp` imports this header for default domain providers.
- Current state: `src/async/` appears to own generic job/callback/scheduler infrastructure, but `imageiojobs` still owns navigation/archive candidate defaults. Stored image data loading now lives in `src/decoding/imagedataloading.*`, and an architecture-boundary test keeps decode request and `KIO::storedGet` ownership out of `src/async/imageiojobs.*`.
