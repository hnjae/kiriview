...
- Current state: `ApplicationActionRuntime` owns the `ApplicationCommandRouter` instance and typed dispatch entry points for triggered actions and fixed viewer shortcuts. The QML-facing `KiriViewApplication` facade still owns state gathering, policy input assembly, and command port construction before forwarding them to the runtime.
