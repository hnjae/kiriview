...
- Evidence: `src/facade/kiriimagedocument.h:247-345` exposes many viewport commands as `Q_INVOKABLE`s and still owns `KiriView::ImageViewportInteraction m_viewportInteraction` for remaining geometry commands. `src/facade/kiriimagedocument.cpp:567-705` computes zoom step, pan, scan, and nearest-anchor logic in the facade. `src/facade/kiriimagedocument.cpp:823-868` computes anchored zoom content position and sequences `m_runtime->requestManualZoomPercent()` with `requestViewportInteractionContentPosition()`. `src/qml/ImageViewport.qml:126-236` calls facade commands and explicitly calls `applyViewportProjection()` after many commands.
- Current state: QML events enter the facade; the facade samples presentation state, computes anchored zoom/pan/scan content positions, calls runtime mutation, and then QML applies the projection. `ImageDocumentRuntime` now owns pending displayed-image final-scan start state, updates it from published document changes, and serves the displayed-image initial content-position command. Focused facade-boundary and runtime coverage now cover centered anchored zoom, pan/final scan commands, and next-displayed-image final scan start handoff.
- Design concern: `KiriImageDocument` still acts as a stateful presentation command planner instead of a thin facade for anchored zoom, pan, scan, and nearest-anchor logic.
...
- Suggested migration: Keep the QML API while moving the remaining anchored zoom, pan, scan, and nearest-anchor calculations into runtime-level methods and moving the remaining `ImageViewportInteraction` geometry-command ownership into presentation runtime/controller.
- Acceptance criteria: `KiriImageDocument` no longer owns `ImageViewportInteraction`; QML does not need to apply viewport projection after command dispatch. Already satisfied: `handleDocumentChanges()` does not mutate viewport interaction state; QML does not need to understand scan-start handoff.
- Priority: P1
