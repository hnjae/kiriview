...
- Current state: Image document command effects, video document source/stop effects, video-output claim lifecycle and attachment effects, direct-media deletion lifecycle, direct-media navigation completion decisions, direct-media predecode coordinator lifecycle, active-navigation thumbnail runtime/source-adaptation lifecycle, active-navigation dispatch operation execution and pending reveal context, source/media route selection and route-operation dispatch, and session-facing image/video leaf snapshot-change publication now have focused boundaries. Leaf snapshot ports are separate from leaf command/effect ports. Completion-route application and session projection/routing remain tied to the same session runtime.
