...
- Suggested migration: Continue extracting small workflows such as route execution and active-navigation dispatch. Keep public projection updates centralized in `DocumentSessionState` and move lifecycle execution/cancellation into subowners.
...
- Current state: Video-output claim lifecycle, direct-media deletion lifecycle, direct-media navigation completion decisions, direct-media predecode coordinator lifecycle, active-navigation thumbnail runtime/source-adaptation lifecycle, and session-facing image/video leaf snapshot-change publication now have focused boundaries. Direct-media route execution, active-navigation dispatch, leaf command/effect callbacks, and session projection/routing remain tied to the same session runtime.
...
- Suggested migration: Continue extracting small effect-boundary features, proceeding next to direct route execution or active-navigation dispatch.
