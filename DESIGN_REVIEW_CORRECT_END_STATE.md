...
- Current state: Video-output claim lifecycle, direct-media deletion lifecycle, direct-media navigation completion decisions, direct-media predecode coordinator lifecycle, active-navigation thumbnail runtime/source-adaptation lifecycle, active-navigation dispatch operation execution and pending reveal context, route-operation dispatch, and session-facing image/video leaf snapshot-change publication now have focused boundaries. Leaf snapshot ports are separate from leaf command/effect ports. Route selection/logging, command/effect port adaptation, and session projection/routing remain tied to the same session runtime.
