...
- Current state: QML events enter the facade; the facade converts QML-friendly inputs and forwards commands. `ImageDocumentRuntime` owns pan/scan content-position commands, anchored zoom positioning, nearest-anchor and point-inside-image queries, pending displayed-image final-scan start state, document-change scan-start updates, and displayed-image initial content-position commands. QML still applies viewport projections after command dispatch.
- Design concern: Viewport command planning now has the correct runtime owner, but command application remains split because QML still decides when to apply the runtime's projected viewport command state.
...
- Suggested migration: Keep the QML API while moving viewport command projection application out of QML into a presentation runtime/controller path that observes command revisions and applied positions.
- Acceptance criteria: QML does not need to apply viewport projection after command dispatch. Already satisfied: `KiriImageDocument` no longer owns `ImageViewportInteraction`; pan/scan commands are runtime-owned; anchored zoom and point-query geometry are runtime-owned; `handleDocumentChanges()` does not mutate viewport interaction state; QML does not need to understand scan-start handoff.
- Priority: P1
