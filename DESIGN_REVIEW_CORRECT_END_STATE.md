...
- Evidence: `src/decoding/imagedecodejob.cpp` has stale no-delivery paths while now warning for empty requests and the defensive missing-dependency guard; `src/document/imageloader.cpp` quietly ignores stale no-current callbacks; `src/async/directorylistingjob.cpp` still reports empty callback text for backend listing failures while now warning for invalid empty URLs and synchronous `openUrl` refusal; `src/qml/ImageToolBar.qml` now warns when application-menu anchor coordinate mapping throws.
- Current state: Expected stale/cancel paths and unexpected backend failure paths can both look like quiet no-ops. Decode empty requests and the defensive missing-dependency guard now emit KiriView warning diagnostics before preserving no-delivery behavior. Directory listing now rejects empty URLs before `KCoreDirLister`, keeps the public callback text empty, and emits a KiriView warning diagnostic; the synchronous `openUrl` false path also emits a warning diagnostic before preserving the existing empty callback text. Toolbar application-menu anchor mapping failures still fall back to the non-anchored menu placement path while now producing a QML warning diagnostic.
