...
- Evidence: `src/facade/kiriviewapplication.h:177-215` owns action-state source subscriptions, the snapshot adapter, command port builders, and UI gate fields. `src/facade/kiriviewapplication.cpp:329-361` rebuilds action state and subscribes to sources; `:395-455` assembles `ApplicationActionStateSnapshot`; `:457-631` builds concrete command router ports. `src/application/applicationactionruntime.h:30-145` owns `ApplicationActionStateSnapshot`, `ApplicationCommandRouter`, `ImageActionAvailabilityProjection`, `ApplicationActionStateInput`, and `ApplicationCommandRouterInput`; `src/application/applicationactionruntime.cpp:35-128` assembles image/action-state inputs and command-router input from the snapshot.
- Current state: `ApplicationActionRuntime` owns the `ApplicationCommandRouter` instance, typed dispatch entry points for triggered actions and fixed viewer shortcuts, image-action availability projection, `ApplicationActionStateInput` assembly, and `ApplicationCommandRouterInput` assembly from facade-supplied session/UI/video snapshots. The QML-facing `KiriViewApplication` facade still owns source subscriptions, snapshot gathering, and command port construction before forwarding ports to the runtime.
