...
- Suggested migration: Continue extracting small workflows such as route execution, active-navigation dispatch, and leaf notification families. Keep public projection updates centralized in `DocumentSessionState` and move lifecycle execution/cancellation into subowners.
...
- Current state: Video-output claim lifecycle, direct-media deletion lifecycle, direct-media navigation completion decisions, direct-media predecode coordinator lifecycle, and active-navigation thumbnail runtime/source-adaptation lifecycle now have focused subowners, and session-facing image/video leaf reads now cross cohesive snapshots. Direct-media route execution, active-navigation dispatch, individual leaf notification hooks, leaf command/effect callbacks, and session projection/routing remain tied to the same session runtime.
...
- Suggested migration: Continue extracting small effect-boundary features, proceeding next to direct route execution, active-navigation dispatch, or leaf snapshot-change event families.
