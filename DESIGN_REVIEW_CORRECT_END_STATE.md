...
- Evidence: `src/facade/kiriviewapplication.h:177-215` owns action-state source subscriptions, the snapshot adapter, concrete `ApplicationCommandPortSource` methods, and UI gate fields. `src/facade/kiriviewapplication.cpp:331-357` rebuilds action state and subscribes to sources; `:390-438` assembles `ApplicationActionStateSnapshot`; `:440-634` adapts concrete shell/session/image/video command port sources. `src/application/applicationactionruntime.h:100-136` owns accepted action snapshots, command-router input, runtime dispatch entry points, and command-router port assembly from an `ApplicationCommandPortSource`; `src/application/applicationcommandportsource.h:18-33` defines the typed source boundary.
- Current state: `ApplicationActionRuntime` owns the `ApplicationCommandRouter` instance, typed dispatch entry points for triggered actions and fixed viewer shortcuts, image-action availability projection, `ApplicationActionStateInput` assembly, `ApplicationCommandRouterInput` assembly, and command-router port assembly from a typed source. The QML-facing `KiriViewApplication` facade still owns source subscriptions, snapshot gathering, and concrete shell/session/image/video port-source adaptation.
...
- Acceptance criteria: `KiriViewApplication` no longer implements action-state source subscriptions or `ApplicationCommandPortSource`; action-state rebuild tests target the application runtime/coordinator rather than the facade.
- Priority: P2
