...
auto backendStatus(QMediaPlayer::MediaStatus status) -> VideoThumbnailBackendMediaStatus
{
    switch (status) {
...
auto backendError(QMediaPlayer::Error error) -> VideoThumbnailBackendError
{
    switch (error) {
...
public:
    QtVideoThumbnailBackend()
    {
        ...
        QObject::connect(
            &player_, &QMediaPlayer::metaDataChanged, &player_, [this]() { publishMetadata(); });
        QObject::connect(&player_, &QMediaPlayer::mediaStatusChanged, &player_,
            [this]() { publishMediaFacts(); });
        QObject::connect(
            &player_, &QMediaPlayer::durationChanged, &player_, [this]() { publishMediaFacts(); });
        QObject::connect(
            &player_, &QMediaPlayer::seekableChanged, &player_, [this]() { publishMediaFacts(); });
        QObject::connect(
            &player_, &QMediaPlayer::hasVideoChanged, &player_, [this]() { publishMediaFacts(); });
        QObject::connect(
            &player_, &QMediaPlayer::positionChanged, &player_, [this](qint64 positionMsec) {
                publishMetadata();
        ...
        QObject::connect(&player_, &QMediaPlayer::errorOccurred, &player_,
            [this](QMediaPlayer::Error error, const QString& diagnostic) {
                ...
                if (callbacks_.errorOccurred) {
                    callbacks_.errorOccurred(backendError(error), diagnostic);
                }
            });
    }
...
private:
    void publishMediaFacts()
    {
        ...
        if (player_.mediaStatus() == QMediaPlayer::InvalidMedia) {
            if (player_.error() != QMediaPlayer::NoError && callbacks_.errorOccurred) {
                callbacks_.errorOccurred(backendError(player_.error()), player_.errorString());
            } else {
                    ...
                    [this]() {
                        ...
                        if (player_.error() != QMediaPlayer::NoError && callbacks_.errorOccurred) {
                            callbacks_.errorOccurred(
                                backendError(player_.error()), player_.errorString());
                            return;
                        }
    ...
    void publishCurrentMediaFacts()
    {
        if (callbacks_.mediaFactsChanged) {
            callbacks_.mediaFactsChanged(
                VideoThumbnailBackendMediaFacts { backendStatus(player_.mediaStatus()),
    ...
    void publishMetadata()
    {
        ...
        if ((cover.isNull() && thumbnail.isNull()) || !callbacks_.metadataAvailable) {
            return;
        }
