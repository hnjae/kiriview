...
The C++ API exposes `ImageViewport::completeProviderCleanupForApplicationShutdown()` as the application-owner completion boundary for provider resources. The caller invokes it on the `QCoreApplication` affinity after destroying every `ImageViewport` and all Qt Quick windows and scene-graph resources that could retain viewport payloads, while the application object and every transferred provider session affinity remain dispatch-capable. The call synchronously waits for accepted provider handles to finish release and for their sessions to finish affinity-correct close and destruction. It returns `true` only when all component-owned provider cleanup has converged; `false` reports that a live viewport owner, retained payload, unavailable affinity, or invalid calling context prevented completion. The boundary does not resume viewport state delivery and does not require the main application event loop to remain entered.
