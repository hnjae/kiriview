# Specification Index

This directory indexes the final user-facing behavior and public API contracts. These files describe externally observable guarantees only; implementation details, project planning, and verification mechanics belong elsewhere.

- [ImageViewport](image-viewport.md)
- [ImageViewport API](image-viewport-api.md)
- [ImageViewport Presentation](image-viewport-presentation.md)
- [ImageViewport State](image-viewport-state.md)
- [ImageSequence Construction](image-sequence.md)
- [ImageSequence Provider Protocol](image-sequence-provider-protocol.md)
- [ImageSequence Provider Adapter](image-sequence-provider-adapter.md)
- [ImageViewport Packaging](image-viewport-packaging.md)
