...
Render failure acknowledgements carry a structured internal cause for operational attribution. The render host must not choose public status or transition behavior; the engine admits only an acknowledgement matching the active render identity and applies the terminal, restoration, and auxiliary-refinement guarantees in the behavior and state specifications. Candidate ownership is released or retained according to that admitted outcome, and stale failures cannot affect newer request or display state.
