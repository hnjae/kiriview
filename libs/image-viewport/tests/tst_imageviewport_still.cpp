...
static ImageViewportCommandOutcome setMirrorCommand(
...
static ImageViewportCommandOutcome setRotationCommand(ImageViewport& item, int degrees)
{
    ImageViewportPresentationCommand command;
...
void ImageViewportStillTest::resetViewResetsTransformsAndPreservesNonTransformPresentationState()
{
    ...
    QCOMPARE(setRotationCommand(item, 90), ImageViewportCommandOutcome::Accepted);
    QCOMPARE(setMirrorCommand(item, true, true), ImageViewportCommandOutcome::Accepted);
...
void ImageViewportStillTest::stillImageSequenceAssignmentPublishesReadyState()
{
    ...
    QCOMPARE(containsPrimaryPagePoint(item, 8.0, 4.0), true);
    QCOMPARE(containsPrimaryPagePoint(item, 16.0, 4.0), false);
    QCOMPARE(containsPrimaryPagePoint(item, 8.0, 8.0), false);
...
void ImageViewportStillTest::clearPreservesPresentationState()
{
    ...
    QCOMPARE(item.state().presentation().zoomPercent(), 0.0);
    QCOMPARE(item.state().presentation().preferredManualZoomPercent(), 250.0);
    QCOMPARE(contentPosition(item), QPointF());
    QCOMPARE(item.state().presentation().looping(), true);
}
...
void ImageViewportStillTest::stillImageReplacementPreservesPresentationState()
{
    ...
    QCOMPARE(setMirrorCommand(item, true, true), ImageViewportCommandOutcome::Accepted);
    QCOMPARE(setBackgroundCommand(
                 item, ImageViewportBackgroundMode::Checkerboard, QColor(20, 40, 60, 255)),
        ImageViewportCommandOutcome::Accepted);
    QCOMPARE(
        setPreferredManualZoomPercentCommand(item, 150.0), ImageViewportCommandOutcome::Accepted);
    QCOMPARE(setLoopingCommand(item, true), ImageViewportCommandOutcome::Accepted);
...
void ImageViewportStillTest::secondaryStillImagePositionSeekReportsUnsupported()
{
    ...
    QCOMPARE(item.seekToPosition(ImageViewportPageRole::Secondary, 0).outcome(),
        ImageViewportCommandOutcome::Unsupported);
    QCOMPARE(
        commandReasonValue(item), enumValue(metaObject, "CommandReason", "UnsupportedRequest"));
    QCOMPARE(revisionTokenProperty(item, "requestRevision"), requestRevision);
...
void ImageViewportStillTest::coordinateHelpersRejectNonFiniteInputs()
{
    ...
    QCOMPARE(containsPrimaryPagePoint(item, infinity, 4.0), false);
    QCOMPARE(containsPrimaryPagePoint(item, 8.0, nan), false);
}
...
void ImageViewportStillTest::stillImageMirroredCoverUsesMirroredVisibleImageRect()
{
    ...
    QCOMPARE(setFitModeCommand(item, ImageViewportFitMode::FitHeight),
        ImageViewportCommandOutcome::Accepted);
    QCOMPARE(setPanDelta(item, QPointF(-50.0, 0.0)), ImageViewportCommandOutcome::Accepted);
    QCOMPARE(setMirrorCommand(item, true, false), ImageViewportCommandOutcome::Accepted);

    QCOMPARE(contentRect(item), QRectF(-100.0, 0.0, 200.0, 100.0));
    QCOMPARE(visibleImageRect(item), QRectF(0.0, 0.0, 8.0, 8.0));
    QCOMPARE(containsPrimaryPagePoint(item, 7.999, 4.0), true);
    QCOMPARE(containsPrimaryPagePoint(item, 8.0, 4.0), true);
    QCOMPARE(containsPrimaryPagePoint(item, 15.999, 4.0), true);
...
void ImageViewportStillTest::stillImageCoverUsesBottomAlignmentAsCropFocus()
{
    ...
    item.setPresentationTarget(
        ImageViewportPresentationTarget(result->sequence()), PresentationTargetTransitionPolicy {});
    acknowledgePendingRenderCommitForTest(item);
    QCOMPARE(setFitModeCommand(item, ImageViewportFitMode::FitWidth),
        ImageViewportCommandOutcome::Accepted);
    ImageViewportPresentationCommand scanCommand;
    scanCommand.setContentAnchor(ImageViewportContentAnchor::End);
    QCOMPARE(item.setPresentation(scanCommand).outcome(), ImageViewportCommandOutcome::Accepted);
    ...
    QCOMPARE(containsPrimaryPagePoint(item, 4.0, 7.999), true);
    QCOMPARE(containsPrimaryPagePoint(item, 4.0, 8.0), true);
    QCOMPARE(containsPrimaryPagePoint(item, 4.0, 15.999), true);
}
...
void ImageViewportStillTest::stillImageAssignmentWaitsForPositiveGeometry()
{
    ...
    QCOMPARE(containsPrimaryPagePoint(item, 8.0, 4.0), false);
}
...
void ImageViewportStillTest::stillImageFactoryRejectsPublishedLimitViolations()
{
    ...
    QVERIFY(result->errorString().contains(QStringLiteral("maximumSourceLogicalWidth")));
}
