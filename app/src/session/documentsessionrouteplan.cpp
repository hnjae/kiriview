...
namespace {
enum class ImageDocumentEnterMode {
    SourceAssignment,
    SameScopeNavigation,
};
    ...
    kiriview::DocumentSessionRoutePlan& plan, kiriview::DocumentSessionKind currentKind)
{
    if (currentKind == kiriview::DocumentSessionKind::Image) {
        plan.mutations.emplace_back(kiriview::RequestDirectImageCursorRouteOperation {});
        return;
    }

    plan.mutations.emplace_back(kiriview::ClearThenRequestDirectImageCursorRouteOperation {});
}

void appendEnterImageDocumentMutations(
    kiriview::DocumentSessionRoutePlan& plan, ImageDocumentEnterMode enterMode)
{
    plan.mutations.emplace_back(kiriview::LeaveVideoModeRouteOperation {});
    switch (enterMode) {
    case ImageDocumentEnterMode::SourceAssignment:
        plan.mutations.emplace_back(kiriview::EnterImageDocumentRouteOperation {});
        return;
    case ImageDocumentEnterMode::SameScopeNavigation:
        plan.mutations.emplace_back(
            kiriview::EnterImageDocumentSameScopeNavigationRouteOperation {});
        return;
    }
}

void appendImageDocumentRoute(kiriview::DocumentSessionRoutePlan& plan,
    ImageDocumentEnterMode enterMode, bool refreshDirectMediaNavigation)
{
    appendEnterImageDocumentMutations(plan, enterMode);
    plan.mutations.emplace_back(kiriview::SyncDirectImageCursorFromDocumentRouteOperation {});
    plan.mutations.emplace_back(kiriview::UseImageDocumentSourceIdentityRouteOperation {});
    plan.publishPublicProjection = true;
    if (refreshDirectMediaNavigation) {
        plan.followUpEffects.emplace_back(
...
kiriview::DocumentSessionRoutePlan emptyRoutePlan(const QUrl& sourceUrl)
{
    ...
    plan.mutations.emplace_back(kiriview::ClearDirectMediaCursorRouteOperation {});
    plan.mutations.emplace_back(kiriview::LeaveVideoModeRouteOperation {});
    plan.mutations.emplace_back(kiriview::ClearImageDocumentRouteOperation {});
    plan.mutations.emplace_back(kiriview::EnterEmptyDocumentRouteOperation {});
    plan.mutations.emplace_back(kiriview::ClearSourceIdentityRouteOperation {});
    plan.publishPublicProjection = true;
    plan.followUpEffects.emplace_back(kiriview::ClearMediaPredecodeRouteEffect {});
    return plan;
}
...
kiriview::DocumentSessionRoutePlan directVideoRoutePlan(const QUrl& sourceUrl)
{
    ...
    plan.sourceUrl = sourceUrl;
    plan.mutations.emplace_back(kiriview::SetDirectVideoCursorRouteOperation {});
    plan.mutations.emplace_back(kiriview::ClearImageDocumentRouteOperation {});
    plan.mutations.emplace_back(kiriview::EnterVideoDocumentRouteOperation {});
    plan.mutations.emplace_back(kiriview::UseOriginalSourceIdentityRouteOperation {});
    plan.publishPublicProjection = true;
    plan.followUpEffects.emplace_back(
        kiriview::RefreshDirectMediaNavigationAfterRoutingRouteEffect {});
    return plan;
...
kiriview::DocumentSessionRoutePlan directImageRoutePlan(const QUrl& sourceUrl,
    kiriview::DocumentSessionKind currentKind, ImageDocumentEnterMode enterMode)
{
    kiriview::DocumentSessionRoutePlan plan;
    plan.kind = kiriview::DocumentSessionRouteKind::DirectImage;
    plan.sourceUrl = sourceUrl;
    appendDirectImageCursorOperation(plan, currentKind);
    appendImageDocumentRoute(plan, enterMode, true);
    return plan;
}
...
kiriview::DocumentSessionRoutePlan imageDocumentRoutePlan(const QUrl& sourceUrl)
{
    ...
    plan.sourceUrl = sourceUrl;
    plan.mutations.emplace_back(kiriview::ClearDirectMediaCursorRouteOperation {});
    appendImageDocumentRoute(plan, ImageDocumentEnterMode::SourceAssignment, false);
    return plan;
}
    ...
    const QUrl& sourceUrl, kiriview::DocumentSessionKind currentKind)
{
    ...
    if (kiriview::isSupportedDirectImageUrl(sourceUrl)) {
        return directImageRoutePlan(
            sourceUrl, currentKind, ImageDocumentEnterMode::SourceAssignment);
    }
    ...
    const QUrl& url, DocumentSessionKind currentKind)
{
    ...
    if (kiriview::isSupportedDirectImageUrl(url) && currentKind == DocumentSessionKind::Image) {
        plan = directImageRoutePlan(url, currentKind, ImageDocumentEnterMode::SameScopeNavigation);
    } else {
        plan = baseRoutePlan(url, currentKind);
    }

    prependMutations(plan,
        std::vector<DocumentSessionRouteMutation> {
            DocumentSessionRouteMutation { CancelMediaDeletionRouteOperation {} },
        });
    return plan;
}
