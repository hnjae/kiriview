...
The repository broad-gate scheduling rule governs when the app completion checks below become mandatory. The same command may run earlier when it is the smallest relevant focused check.
...
| Intent-only commit (spec / architecture / expected-failing regression test before implementation)                  | Run only the relevant documentation check or expected-failing focused behavior test. Do not run completion groups for this intermediate commit; apply the repository early-gate exceptions only when needed.                                                          |
| Final code handoff candidate                                                                                       | Run the checks in every applicable changed-surface row below; do not add unrelated language or packaging groups.                                                                                                                                                      |
...
Report every applicable completion check that was not run and why. Report focused checks that were attempted but could not be completed.
