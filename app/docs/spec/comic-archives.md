...
When navigation in Two-Page Spread targets another eligible two-page spread, KiriView accepts the target while keeping the previous complete page or spread visible as a non-interactive fallback. It never presents only one page of the target spread.
...
While a Two-Page Spread navigation transition is loading, the previously displayed complete page or spread remains the last committed image presentation until the complete target is ready. KiriView must not expose a partially prepared target spread as the active image presentation.

If a Two-Page Spread navigation replacement fails, KiriView discards the fallback and shows the selected target's error state instead of restoring the previous presentation.
