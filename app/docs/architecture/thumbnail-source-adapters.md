...
Video thumbnail extraction remains behind a provider boundary. KiriView supplies an eligible authorized source and requested bound, admits the provider's declared one-operation pixel working set against the shared decoded-memory policy before invoking it, keeps caller-owned access prerequisites and that admission valid while an extraction job is active, and maps the typed result into application failure and publication policy. A ready result replaces the working admission with its retained raster charge, which follows the final pixel alias. Stable provider failure causes remain distinguishable as bounded application-owned categories through the operational diagnostic boundary without parsing diagnostic text; this diagnostic distinction does not change placeholder fallback, cache, publication, or user-facing background-error policy. KiriView treats the extraction job as an opaque asynchronous capability and must not depend on provider implementation mechanisms or borrow application playback state for extraction.
