...
- Application-owned terminal diagnostics preserve record framing, neutralize control and format content, and enforce a fixed encoded-byte bound before writing to the sink. User-facing startup diagnostics may retain a bounded visible path representation when the product contract requires it, but truncation and escaping must remain unambiguous.
