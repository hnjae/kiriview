...
The image-document state owner holds the session-scoped toolbar-selected fit preference because that application interaction fact is distinct from the dependency's active fit mode. The preference starts at Fit, changes after an accepted non-manual fit or reset command, remains unchanged by manual zoom and same-navigation-scope or presentation-shape transitions, and resets to Fit only when an outside-navigation-scope target becomes the matching committed presentation. A failed or restored replacement preserves the prior preference. For a matching committed current presentation whose active fit mode is non-manual, the active mode and selected preference agree.
...
The toolbar and action runtime derive active fit action state, zoom readout, editability, pannability, and image readiness from current matched public observations and consume the image-document-owned selected fit preference for the fit menu button. A retained toolbar appearance is a readonly projection of the last complete presentation rather than an independent applied zoom, fit preference, or action state.
