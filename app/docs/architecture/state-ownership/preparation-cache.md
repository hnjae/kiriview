...
Still-image and animation production across foreground loading, both preparation owners, provider work, and image-thumbnail work consume one asynchronous, priority-aware application decoded-memory admission policy. Selected-image and visible-frame or detail work precedes visible auxiliary work, which precedes speculative preparation. Every supported route must satisfy both per-operation and aggregate bounds, or a proven equivalent decoder cap, before materializing input-derived decoder canvas, frame, raster, or application-copy storage; production begins only while holding a current grant. A request that intrinsically exceeds a hard bound remains a typed failure through the owning boundary, while a feasible request blocked only by current aggregate occupancy remains pending as backpressure without publishing exhaustion. Decoder and transient reservations do not authorize returned image storage: every simultaneously retained still raster or animation frame keeps an aggregate charge, including while another result is produced. Prepared still payloads transfer that charge through foreground reuse instead of acquiring a second reservation, and every retained pixel alias preserves it until physical retirement. A decoded-memory admission cannot authorize concurrent operations, and its accounting does not replace the cache, provider-payload, or display-store ownership and eviction policy for published outputs. Cancellation may suppress publication immediately, but canceled running work remains charged and keeps its work identity until physical retirement, so it cannot admit replacement work early.
