...
Animation decode and composition across foreground loading, both preparation owners, provider animation work, and image-thumbnail work consume one application decoded-workspace admission policy. Every supported animation route must satisfy both per-operation and aggregate bounds, or a proven equivalent decoder cap, before materializing input-derived decoder canvas, frame, or application-copy storage; admitted storage remains charged until its physical release, and resource exhaustion remains a typed failure through the owning boundary. Decoder and transient reservations do not authorize returned frame storage: every simultaneously retained frame output keeps an independent aggregate charge, including while a later frame is produced. A workspace admission cannot authorize concurrent operations, and transient workspace accounting does not replace the cache, provider-payload, or display-store ownership of published outputs. Each provider source admits at most one animation-frame production through terminal handoff; cancellation may suppress publication but cannot admit replacement work before the prior production has retired.
