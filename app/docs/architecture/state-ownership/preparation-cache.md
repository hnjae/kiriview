...
Still-image and animation production across foreground loading, both preparation owners, provider work, and image-thumbnail work consume one application decoded-memory admission policy. Every supported route must satisfy both per-operation and aggregate bounds, or a proven equivalent decoder cap, before materializing input-derived decoder canvas, frame, raster, or application-copy storage; admitted storage remains charged until its physical release, and resource exhaustion remains a typed failure through the owning boundary. Decoder and transient reservations do not authorize returned image storage: every simultaneously retained still raster or animation frame keeps an aggregate charge, including while another result is produced. Prepared still payloads transfer that charge through foreground reuse instead of acquiring a second reservation, and every retained pixel alias preserves it until physical retirement. A decoded-memory admission cannot authorize concurrent operations, and its accounting does not replace the cache, provider-payload, or display-store ownership and eviction policy for published outputs. Cancellation may suppress publication but cannot admit replacement work before the prior production has retired.
