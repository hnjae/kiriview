...
Direct-media scope identity contains the normalized current and parent identities plus owner-issued freshness evidence. The current and parent identities use the navigation URL and parent location sealed into the active navigation-source snapshot before applying source-key normalization. Only an authenticated document-portal host-path fact may replace the requested URL when those identities are formed; ordinary or unverifiable file metadata cannot grant a different scope identity. Platform probes for document-portal host paths and platform archive-entry restoration are source-entry adapter work performed once per accepted source snapshot; source-key construction and equality consume that snapshot and never trigger platform probes. Replacing or discarding the active source snapshot is the invalidation boundary for both positive and negative platform facts.
