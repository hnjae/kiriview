...
The image-document graph composes navigation, loading, `ImageViewport` integration, sequence-provider resources, preparation, and removal-fallback owners. The repository-internal viewport component is a child with its own canonical presentation state, not another application runtime owner. Each dependent application owner receives its prerequisite ports explicitly. A collaborator that needs a sibling snapshot or command must use a named port rather than capture the sibling merely to inspect or mutate its state.

Provider-resource composition separates application source identity, decode and cache reuse, entry retention, refinement work, and failure records from component presentation. Provider owners receive source, scope, page-role, component request, and demand identities through their ports, create failure handles for typed application records, and accept exact-once handle release so they can enforce reuse, cancellation, stale rejection, and lifetime without becoming presentation or render owners.
